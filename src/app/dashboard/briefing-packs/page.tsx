...
export default function BriefingPacksPage() {
  ...
  function renderSection() {
    ...
    switch (activeSection) {
      case "backgroundNote": return <div style={{ lineHeight: 1.8, fontSize: 13, color: "#E8EFF8" }}>{s.backgroundNote}</div>;
      case "whyFitsTBP": return <div style={{ lineHeight: 1.8, fontSize: 13, color: "#E8EFF8" }}>{s.whyFitsTBP}</div>;
      case "relevantTBPThemes": return <ul style={{ margin: 0, paddingLeft: 20 }}>{s.relevantTBPThemes.map((t, i) => <li key={i} style={{ fontSize: 13, color: "#E8EFF8", marginBottom: 8 }}>{t}</li>)}</ul>;
      case "suggestedConversationAngle": return <div style={{ lineHeight: 1.8, fontSize: 13, color: "#E8EFF8" }}>{s.suggestedConversationAngle}</div>;
      ...
      case "likelyQuestions": return <ol style={{ margin: 0, paddingLeft: 20 }}>{s.likelyQuestions.map((q, i) => <li key={i} style={{ fontSize: 13, color: "#E8EFF8", marginBottom: 8 }}>{q}</li>)}</ol>;
      case "recommendedContactRoute": return <div style={{ lineHeight: 1.8, fontSize: 13, color: "#E8EFF8" }}>{s.recommendedContactRoute}</div>;
      case "draftIntroEmail": return <pre style={{ whiteSpace: "pre-wrap", fontFamily: "inherit", fontSize: 12, color: "#E8EFF8", background: "rgba(255,255,255,0.04)", padding: 16, borderRadius: 8, lineHeight: 1.7, border: "1px solid rgba(255,255,255,0.06)" }}>{s.draftIntroEmail}</pre>;
      case "draftFollowUpEmail": return <pre style={{ whiteSpace: "pre-wrap", fontFamily: "inherit", fontSize: 12, color: "#E8EFF8", background: "rgba(255,255,255,0.04)", padding: 16, borderRadius: 8, lineHeight: 1.7, border: "1px solid rgba(255,255,255,0.06)" }}>{s.draftFollowUpEmail}</pre>;
      case "suggestedMeetingAgenda": return <ol style={{ margin: 0, paddingLeft: 20 }}>{s.suggestedMeetingAgenda.map((item, i) => <li key={i} style={{ fontSize: 13, color: "#E8EFF8", marginBottom: 8 }}>{item}</li>)}</ol>;
      case "postMeetingActionTemplate": return <ul style={{ margin: 0, paddingLeft: 20 }}>{s.postMeetingActionTemplate.map((item, i) => <li key={i} style={{ fontSize: 13, color: "#E8EFF8", marginBottom: 8 }}>{item}</li>)}</ul>;
      ...
      <div style={{ padding: "24px 28px 40px" }}>
        ...
        <Panel title="Select Prospect" subtitle="Choose a scored prospect to generate an internal briefing pack" style={{ marginBottom: 14 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 12, alignItems: "end" }}>
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: "#4A5C70", textTransform: "uppercase", letterSpacing: ".8px", marginBottom: 6, display: "block" }}>
                Eligible Prospects (Score 65+)
              </label>
              <select
                style={{ width: "100%", padding: "9px 12px", borderRadius: 8, border: "1px solid rgba(255,255,255,0.12)", fontSize: 13, color: "#E8EFF8", background: "#0A1624", outline: "none" }}
                value={selectedId}
            ...
            <button
              ...
              style={{
                background: selectedId && !generating ? "#C4992A" : "rgba(255,255,255,0.08)",
                color: selectedId && !generating ? "#0C1929" : "#4A5C70",
          ...
          {selected && (
            <div style={{ marginTop: 16, padding: "14px 16px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 8, display: "flex", gap: 20 }}>
              <div>
                <div style={{ fontSize: 11, color: "#4A5C70", marginBottom: 2 }}>PROSPECT</div>
                <div style={{ fontWeight: 700, fontSize: 14, color: "#E8EFF8" }}>{selected.prospect_name}</div>
                <div style={{ fontSize: 12, color: "#7B8EAA" }}>{selected.city}, {selected.country} · {selected.region}</div>
              </div>
              <div>
                <div style={{ fontSize: 11, color: "#4A5C70", marginBottom: 2 }}>SCORE</div>
                <ScoreBadge score={selected.suitability_score} />
              </div>
              <div>
                <div style={{ fontSize: 11, color: "#4A5C70", marginBottom: 2 }}>BEST ENTRY POINT</div>
                <div style={{ fontSize: 12, color: "#E8EFF8" }}>{selected.best_tbp_entry_point}</div>
              </div>
            </div>
          )}
        </Panel>
        ...
        {pack && (
          <div style={{ display: "grid", gridTemplateColumns: "220px 1fr", gap: 14 }}>
            <div style={{ background: "#132037", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, overflow: "hidden", alignSelf: "start" }}>
              <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.06)", fontSize: 12, fontWeight: 700, color: "#E8EFF8" }}>Pack Sections</div>
              <div style={{ padding: 8 }}>
                {sectionNav.map((s) => (
                  <button
                    key={s.key}
                    ...
                    style={{
                      ...
                      color: activeSection === s.key ? "#C4992A" : "#7B8EAA",
